...
SELECT animal_type, outcome_type, CAST(COUNT(outcome_type) as numeric(10,2))/79272 * 100 AS Percentage
FROM Austin_Animal_Center_Outcomes_Cleaned
WHERE animal_type = 'Dog' AND outcome_type IS NOT NULL
GROUP BY animal_type, outcome_type
ORDER BY 3 DESC

/* Dogs and cats have almost the same adoption percentage rate with around 48%. Cats are almost twice more likely to be euthanized with a percentage rate of 4.05%,
compared to dogs with 2.43%. Another big differnce is the return to owner percentage, with dogs having a 29.97% comapared to 4.89% in cats. */
